...
def mid_page():
    ...
    menu_item_list = all_menus[ (all_menus.restaurant ==  rest_name) & (all_menus.address ==  rest_address) ].item_name.tolist()
...
def result():
    ...
    idx = all_menus[(all_menus.restaurant == rest_name) & (all_menus.address == rest_address) & (all_menus.item_name == menu_item)].index[0]
    ...
    for cos_sim,sim_item in zip(menu_cosine_similarities,other_similar_items):
        if (all_menus.iloc[sim_item].address != all_menus.iloc[user_selection].address):
    ...
    for i in top_num_sim_items_list:
        ...
        name_print = all_menus.iloc[i].restaurant
        price_print = all_menus.iloc[i].item_price
        address_print = all_menus.iloc[i].address
        result_dict.append(dict(menu_item=menu_print, desc=desc_print, name=name_print, price=price_print,address = address_print) )
